import type { StickerPack, PackWithStickers, Sticker } from '@/types';
import { getDatabase } from './init';

export function getAllPacks(): StickerPack[] {
  const db = getDatabase();
  const result = db.execute('SELECT * FROM sticker_packs ORDER BY updated_at DESC');
...
export function updatePackTrayImage(id: string, trayImageFile: string): void {
  const db = getDatabase();
  db.execute('UPDATE sticker_packs SET tray_image_file = ?, updated_at = ? WHERE id = ?', [
    trayImageFile,
    Date.now(),
    id,
  ]);
...
export function getStickerCountForPack(packId: string): number {
  const db = getDatabase();
  const result = db.execute('SELECT COUNT(*) as count FROM stickers WHERE pack_id = ?', [packId]);
  const rows = result.rows?._array || [];
  return rows.length > 0 ? (rows[0] as Record<string, number>).count : 0;
}
