...
import { View, FlatList, Alert, Dimensions, TouchableOpacity } from 'react-native';
...
import { getStickerPackDetail, downloadStickerToFile } from '@/services/sigstickApi';
...
import { generateTrayIcon, convertToStickerWebP, TRAY_FILE_NAME } from '@/services/imageProcessor';
...
export default function SigStickResultScreen() {
  ...
  const { packId, packTitle: paramTitle, stickers: paramStickers } = useLocalSearchParams<{ packId: string; packTitle?: string; stickers?: string }>();
  const t = useTheme();
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      Alert.alert('Success', `"${packTitle}" has been added to your packs!`, [
        { text: 'View Pack', onPress: () => router.replace('/pack-detail?packId=' + identifier) },
      ]);
    } catch (e: any) {
      setDownloading(false);
  ...
  const renderItem = ({ item }: { item: string }) => (
    <View style={{ width: ITEM_SIZE, height: ITEM_SIZE, backgroundColor: t.colors.surface, borderRadius: 8, overflow: 'hidden', alignItems: 'center', justifyContent: 'center', marginBottom: 8 }}>
      <Image source={{ uri: item }} style={{ width: ITEM_SIZE * 0.85, height: ITEM_SIZE * 0.85 }} contentFit="contain" />
    </View>
  );
  ...
  if (loading) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Text variant="bodyLarge" style={{ color: t.colors.onSurfaceVariant }}>Loading pack...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.colors.background }}>
      ...
      <Text variant="headlineSmall" style={{ fontWeight: '700', color: t.colors.onSurface, paddingHorizontal: 16, paddingTop: 16 }}>{packTitle}</Text>
      <Text variant="bodySmall" style={{ color: t.colors.onSurfaceVariant, paddingHorizontal: 16, paddingBottom: 12 }}>{stickerUrls.length} stickers</Text>

      <FlatList data={stickerUrls} renderItem={renderItem} keyExtractor={(item, i) => `${i}`}
        numColumns={NUM_COLUMNS} contentContainerStyle={{ padding: 16 }} columnWrapperStyle={{ gap: 8 }}
      />

      {downloading && <ProgressBar progress={downloadProgress} total={stickerUrls.length} label="Downloading stickers..." />}

      <View style={{ padding: 16, borderTopWidth: 1, borderTopColor: t.colors.outlineVariant }}>
        <Button mode="contained" buttonColor={downloading ? t.colors.surfaceDisabled : t.colors.primary}
          contentStyle={{ paddingVertical: 8 }}
          onPress={handleDownload} disabled={downloading}
          icon={() => <Icon source="download" size={22} color={t.colors.onPrimary} />}
        >
