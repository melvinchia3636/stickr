    ...
    private val activityEventListener = object : BaseActivityEventListener() {
        override fun onActivityResult(activity: android.app.Activity, requestCode: Int, resultCode: Int, data: Intent?) {
            if (requestCode == 200) {
                ...
                Log.d(TAG, "  resultCode: $resultCode (OK=${android.app.Activity.RESULT_OK}, CANCELED=${android.app.Activity.RESULT_CANCELED})")
                if (data != null) {
                    val validationError = data.getStringExtra("validation_error")
                    Log.d(TAG, "  validation_error extra: '$validationError'")
