...
async function createSubPack(
  ...
  console.log(
    `${TAG} createSubPack() part ${partIndex + 1}/${totalParts} subId=${subId} stickers=${stickers.length}`
  )
  ...
  if (trayExists) {
    ...
    console.log(
      `${TAG}   generating tray from first sticker: ${firstStickerSrc}`
    )
  ...
  console.log(
    `${TAG}   subPack dir files: ${files.map(f => `${f.name}(${f.size})`).join(', ')}`
  )
...
export async function prepareSubPacks(
  ...
  console.log(
    `${TAG} prepareSubPacks() identifier=${pack.identifier} totalStickers=${pack.stickers.length}`
  )
        ...
        try {
          ...
          if (trayFile) {
            ...
            if (!(await RNFS.exists(trayPath))) {
              console.log(
                `${TAG}   removing broken contents.json (missing tray ${trayFile}) in: ${dir.name}`
              )
  ...
  console.log(
    `${TAG}   splitting into ${chunks.length} chunks: ${chunks.map(c => c.length).join(', ')}`
  )
  ...
  console.log(
    `${TAG}   parent contents.json exists=${parentExists}, removing...`
  )
  if (parentExists) {
  ...
  console.log(
    `${TAG}   prepareSubPacks() done, ${subPacks.length} sub-packs ready`
  )
  return subPacks
}
...
export async function addSubPackToWhatsApp(subPack: SubPack): Promise<void> {
  console.log(
    `${TAG} addSubPackToWhatsApp() identifier=${subPack.identifier} label="${subPack.label}"`
  )
...
export async function addPackToWhatsApp(pack: PackWithStickers): Promise<void> {
  console.log(
    `${TAG} addPackToWhatsApp() stickers=${pack.stickers.length} needsSplit=${pack.stickers.length > MAX_STICKERS_PER_PACK}`
  )
