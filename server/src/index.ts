...
import {
  convertWithCompressionFallback,
  generateTrayIconServer
} from './converter'
...
function requestLogger(req: Request, res: Response, next: () => void): void {
  ...
  res.on('finish', () => {
    ...
    const durationStyle =
      duration > 5000 ? chalk.red : duration > 1000 ? chalk.yellow : chalk.cyan
...
app.get('/api/convert', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    console.log(
      `[${timestamp}] ${chalk.green('[CONVERT]')} Converted remote URL successfully -> Size: ${Math.round(size / 1024)} KB`
    )
    ...
    console.error(
      `[${timestamp}] ${chalk.red('[ERROR]')} Remote URL conversion failed: ${message}`
    )
...
app.post('/api/convert', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    console.log(
      `[${timestamp}] ${chalk.green('[CONVERT]')} Converted local upload successfully -> Size: ${Math.round(size / 1024)} KB`
    )
    ...
    console.error(
      `[${timestamp}] ${chalk.red('[ERROR]')} Local upload conversion failed: ${message}`
    )
    res.status(500).json({ error: message })
  } finally {
    try {
...
app.get('/api/tray', async (req: Request, res: Response): Promise<void> => {
  const url = req.query.url as string
  ...
  try {
    ...
    console.log(
      `[${timestamp}] ${chalk.green('[TRAY]')} Generated remote URL tray icon successfully -> Size: ${Math.round(size / 1024)} KB`
    )
    ...
    console.error(
      `[${timestamp}] ${chalk.red('[ERROR]')} Remote URL tray icon failed: ${message}`
    )
    res.status(500).json({ error: message })
...
app.post('/api/tray', async (req: Request, res: Response): Promise<void> => {
  const fileData = req.body.fileData as string
  ...
  try {
    ...
    console.log(
      `[${timestamp}] ${chalk.green('[TRAY]')} Generated local upload tray icon successfully -> Size: ${Math.round(size / 1024)} KB`
    )
    ...
    console.error(
      `[${timestamp}] ${chalk.red('[ERROR]')} Local upload tray icon failed: ${message}`
    )
    res.status(500).json({ error: message })
  } finally {
    try {
